...
async function submit(event: Event) {
    ...
    if (valid && member.value) {
        ...
        if (member.value.idx) {
            results = await membersStore.updateMember(member.value.idx, member.value);
        } else {
            // eslint-disable-next-line @typescript-eslint/no-unused-vars
            const { idx, ...memberData } = member.value;
            results = await membersStore.createMember(memberData);
        }
