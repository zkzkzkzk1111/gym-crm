...
async function submit(event: Event) {
    ...
    if (valid && staff.value) {
        ...
        const staffRequest = {
            ...
            phone: staff.value.phone ? Number(staff.value.phone) : null,
            gender: staff.value.gender
        }

        if (staff.value.idx) {
            results = await staffStore.updateStaff(staff.value.idx, staffRequest);
        } else {
            results = await staffStore.createStaff(staffRequest);
        }
